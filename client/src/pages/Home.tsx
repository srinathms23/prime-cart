      ...
      <header className="sticky top-0 z-40 border-b border-[#E7E2DA]/90 bg-[#FFFDF9]/90 backdrop-blur-xl">
        <div className="container flex h-[74px] items-center gap-3 lg:gap-6">
          ...
          <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
            ...
            <button type="button" onClick={() => { if (isAuthenticated) { void logout().then(() => toast.success("Signed out of PRIME CART")); } else { startLogin(); } }} className="relative inline-flex h-10 items-center gap-2 rounded-xl border border-transparent px-2.5 text-[#314047] transition hover:border-[#EAE4DB] hover:bg-white" aria-label={isAuthenticated ? "Sign out of account" : "Join or sign in for free"} title={isAuthenticated ? "Sign out" : "Join or sign in for free"}><CircleUserRound className="h-5 w-5" />{isAuthenticated && <span aria-hidden="true" className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-[#6A866E] ring-2 ring-[#FFFDF9]" />}{!loading && <span className="hidden max-w-[110px] truncate text-xs font-extrabold sm:inline">{isAuthenticated ? `Hi, ${user?.name?.split(" ")[0] ?? "there"}` : "Join free"}</span>}</button>
