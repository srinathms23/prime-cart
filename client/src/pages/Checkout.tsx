...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === "success") {
      toast.success("Payment received", { description: "Thank you — we’ll keep you updated about fulfilment." });
    }
    if (paymentStatus === "cancelled") {
      toast.message("Payment cancelled", { description: "Your cart is still here whenever you’re ready." });
    }
    if (paymentStatus) window.history.replaceState({}, "", "/checkout");
  }, []);
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF9] text-[#17232B]">
      ...
      <main className="container py-9 sm:py-12">
        <div className="mb-8">
          <button type="button" onClick={() => navigate("/")} className="inline-flex items-center gap-2 text-sm font-extrabold text-[#60706D] transition hover:text-[#EF6A3A]"><ArrowLeft className="h-4 w-4" /> Back to shopping</button>
          <p className="eyebrow mt-8">Secure checkout</p>
          <h1 className="section-title mt-2">A few details, then <em>done.</em></h1>
          <p className="mt-3 max-w-xl text-sm leading-6 font-medium text-[#6B7875]">Tell us where this order should go. We’ll open Stripe’s secure hosted payment page for the final step.</p>
        </div>

        {cartItems.length ? (
          <form onSubmit={handleSubmit} className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-start">
            <section className="rounded-[28px] border border-[#E8E0D6] bg-white p-6 shadow-[0_8px_28px_rgba(23,35,43,0.04)] sm:p-8">
              <div className="flex items-start gap-3 border-b border-[#EEE7DE] pb-6"><span className="grid h-10 w-10 place-items-center rounded-[15px] bg-[#F4E7DE] text-[#C9532B]"><MapPin className="h-5 w-5" /></span><div><h2 className="font-display text-[28px] tracking-[-0.04em] text-[#26363D]">Shipping details</h2><p className="mt-1 text-xs font-medium text-[#7A8782]">We use these details only to prepare delivery and prefill secure checkout.</p></div></div>
              <div className="mt-7 grid gap-5 sm:grid-cols-2">
                <Field label="Full name" name="fullName" value={form.fullName} onChange={(value) => setForm((current) => ({ ...current, fullName: value }))} placeholder="Sri Kumar" required />
                <Field label="Email address" name="email" type="email" value={form.email} onChange={(value) => setForm((current) => ({ ...current, email: value }))} placeholder="sri@example.com" required />
                <Field label="Mobile number" name="phone" type="tel" value={form.phone} onChange={(value) => setForm((current) => ({ ...current, phone: value }))} placeholder="98765 43210" required /><div className="hidden sm:block" />
                <Field label="Address" name="address" value={form.address} onChange={(value) => setForm((current) => ({ ...current, address: value }))} placeholder="Door number, street, locality" required className="sm:col-span-2" />
                <Field label="Apartment, suite, etc. (optional)" name="apartment" value={form.apartment} onChange={(value) => setForm((current) => ({ ...current, apartment: value }))} placeholder="Floor or landmark" className="sm:col-span-2" />
                <Field label="City" name="city" value={form.city} onChange={(value) => setForm((current) => ({ ...current, city: value }))} placeholder="Chennai" required />
                <label className="checkout-field"><span>State</span><select value={form.state} onChange={(event) => setForm((current) => ({ ...current, state: event.target.value }))}><option>Tamil Nadu</option><option>Karnataka</option><option>Kerala</option><option>Maharashtra</option><option>Delhi</option></select></label>
                <Field label="Postal code" name="postalCode" value={form.postalCode} onChange={(value) => setForm((current) => ({ ...current, postalCode: value }))} placeholder="600001" required />
              </div>
              <div className="mt-8 border-t border-[#EEE7DE] pt-7"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-[15px] bg-[#E8F0E5] text-[#59775D]"><Truck className="h-5 w-5" /></span><div><h2 className="font-display text-[28px] tracking-[-0.04em] text-[#26363D]">Delivery pace</h2><p className="mt-1 text-xs font-medium text-[#7A8782]">Choose what fits this order.</p></div></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><DeliveryChoice checked={delivery === "standard"} onChange={() => setDelivery("standard")} title="Standard delivery" detail="2–4 business days · Free over ₹2,000" price={shipping === 0 && delivery === "standard" ? "Included" : "₹99"} /><DeliveryChoice checked={delivery === "express"} onChange={() => setDelivery("express")} title="Express delivery" detail="Next business day where available" price="₹149" /></div></div>
            </section>
        ...
        ) : <div className="rounded-[30px] border border-dashed border-[#DCCFC2] bg-[#FBF6F0] px-6 py-20 text-center"><span className="mx-auto grid h-15 w-15 place-items-center rounded-[22px] bg-[#F4E6DC] text-[#C9532B]"><ShoppingBag className="h-7 w-7" /></span><h2 className="font-display mt-6 text-4xl tracking-[-0.05em] text-[#28383E]">Your cart needs a good find.</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 font-medium text-[#71807A]">Add a product before beginning checkout.</p><button type="button" onClick={() => navigate("/")} className="mt-7 inline-flex items-center gap-2 rounded-2xl bg-[#17232B] px-5 py-3.5 text-sm font-extrabold text-white transition hover:bg-[#EF6A3A]"><ShoppingBag className="h-4 w-4" /> Browse the edit <ArrowRight className="h-4 w-4" /></button></div>}
      </main>
    </div>
  );
}
